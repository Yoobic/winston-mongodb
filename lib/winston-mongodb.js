 ...
 * @param {boolean=false} options.skipHandlingIndexes Will skip creating TTL index
 */
let MongoDB = exports.MongoDB = function(options) {
  ...
  this.skipHandlingIndexes = options.skipHandlingIndexes;
  ...
  function createCollection(db) {
    ...
    return db.createCollection(self.collection, opts).then(col=>{
      if (self.skipHandlingIndexes) return;
